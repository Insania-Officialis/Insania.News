...
using Insania.News.Contracts.Services;
using Insania.News.Database.Contexts;
using Insania.News.Entities;

namespace Insania.News.Services;
...
public class LoggingSL: ILoggingSL, IHostedService, IDisposable
{
    ...
    public LoggingSL(IServiceScopeFactory scopeFactory, ILogger<LoggingSL> logger)
    {
        ...
        //Создание канала
        _channel = Channel.CreateBounded<LogApiNews>(options);
    }
    #endregion
    ...
    /// </summary>
    private readonly Channel<LogApiNews> _channel;
    ...
    /// Пачка записей
    /// </summary>
    private readonly List<LogApiNews> _batch = [];
    ...
    public ValueTask QueueLogAsync(LogApiNews log) => _channel.Writer.WriteAsync(log, _cancellationTokenSource.Token);
    ...
    private async Task SaveBatchAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            LogsApiNewsContext context = scope.ServiceProvider.GetRequiredService<LogsApiNewsContext>();
